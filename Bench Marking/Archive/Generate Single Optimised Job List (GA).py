...
#pdb.set_trace()
bench_mark_name = "2018-07-03 04:00:00"
...
while generation <= convergence_generation:
    breeding = 1
    if max_chromosome_size > 1: # only breed or mutate if more than one gene exists
...
best_solution_file_name = GA_output_folder + "best_solution_" + bench_mark_name.replace(":","") + ".csv"
...
best_solution_compiled.to_csv(best_solution_file_name,header=True,index=False)
...
current_storekey = best_solution_compiled.iloc[max_chromosome_size,2].astype(int)
