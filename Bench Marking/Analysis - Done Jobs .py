...
sql_locations =     "SELECT DISTINCT "\
                    "	LocationKeyStartPoint "\
                    "   ,LocationKeyEndPoint "\
                    "   ,MetersBetweenPoints "\
                    "   ,LocationLookupKey "\
                    "FROM LocationDistancesNorm_BenchMarking "\
                    "WHERE LocationKeyEndPoint <> 0"
...
#calculate distances
tech_done_jobsdf = tech_done_jobsdf.append(tech_done_jobsdf.iloc[-1,:],ignore_index=True)
...
#get lookup key into main done jobs df
tech_done_jobsdf = pd.merge(tech_done_jobsdf,location_points_lookup,left_index=True,right_index=True,how="inner")
#get distance
...
tech_done_jobsdf = pd.merge(tech_done_jobsdf,location_points_distance,left_on="LookupKey",right_on="LookupKey",how="left")
...
tech_done_jobsdf.to_csv(GA_output_folder + "done_jobs_results.csv",header=True)
